// Authenticated CRUD for New Opps email schedules, backed by the admin SDK
...
// Mirrors api/pe-opps-schedules.js against the `newOppsEmailSchedules`
// collection.
//
// Body: { action: 'list' }
...
const COLLECTION = 'newOppsEmailSchedules';
...
function buildScheduleDoc(input, auth) {
  ...
  return {
    ...
    recipients,
    subject: String(s.subject || 'New Opportunities').slice(0, 300),
...
async function handler(req, res, auth) {
  ...
  if (!(await enforceRateLimit(res, auth.uid, 'new-opps-schedules', 60, 5 * 60 * 1000))) return;
