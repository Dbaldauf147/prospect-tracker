  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.body}>
        <LeadsCard data={leadsData} hasOpps={hasOpps} />
        <QuotedProjectionsCard data={quotedData} hasOpps={hasOpps} target={target} />
        <CloseRateCard data={closeRateData} hasOpps={hasOpps} />
      </div>
    </div>
  );
}

function LeadsCard({ data, hasOpps }) {
  return (
    <div className={styles.chartCard}>
      <h2 className={styles.chartTitle}>Leads</h2>
      {!hasOpps ? (
...
function QuotedProjectionsCard({ data, hasOpps, target }) {
  const monthlyTarget = target > 0 ? target / 12 : 0;
  const hasAnyValues = data.some(r => r._hasData);
  return (
    <div className={styles.chartCard}>
      <h2 className={styles.chartTitle}>Quoted Projections</h2>
      {!hasOpps ? (
        <div className={styles.empty}>No Opps data — open the Opps tab to load.</div>
      ) : !hasAnyValues ? (
...
function CloseRateCard({ data, hasOpps }) {
  return (
    <div className={styles.chartCard}>
      <h2 className={styles.chartTitle}>Close Rate</h2>
      {!hasOpps ? (
        <div className={styles.empty}>No Opps data — open the Opps tab to load.</div>
      ) : data.length === 0 ? (
        <div className={styles.empty}>No opps with an Open Year value.</div>
