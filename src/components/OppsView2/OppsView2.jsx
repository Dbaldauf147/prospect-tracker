...
// an unmount, or the doc grew past the 1 MB Firestore limit and the
// write failed silently) would clobber a fresh local IDB cache.
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      if (raw?.json) {
        ...
        // Surface the document's stored updatedAt onto the data
        // payload so the hydration timestamp compare works even when
        // the json blob predates the _updatedAt stamp inside it.
...
// Throws when the save fails so callers can decide how loud to be
// (the inline Import path surfaces failures via alert; the debounced
// auto-save path just logs to the console). Firestore caps a single
// document at 1 MB — when Opps 2 grows past that we want the user to
// know rather than silently losing writes.
...
export function OppsView2({ settings, updateSettings, prospects = [], updateProspect } = {}) {
    ...
    try {
      ...
      if (user?.uid) {
        try {
          ...
          firestoreWarning = `\n\nWarning: cross-device sync failed (${err?.message || err}). Rows are safe on this device — IndexedDB has the latest, and hydration now picks the newer of IndexedDB vs Firestore so a refresh here will keep them.`;
        }
      }
      window.alert(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // that didn't make it round-trip (debounce cancelled by unmount,
      // network glitch, Firestore 1 MB doc size cap) don't clobber a
      // fresh local cache on the next load. When both are present,
      // compare _updatedAt and pick the newer; when only one has data,
      // use it directly.
