...
export function SitesView({ settings, updateSettings, prospects = [] } = {}) {
  ...
  async function exportIndicativeSavings({ returnBuffer = false } = {}) {
    ...
    // rather than rolling up to the country level.
    {
      ...
      // province is filled with its market-status color, darkened by
      // its site count (sqrt-scaled, so a single site is still clearly
      // visible while a max-count state hits full saturation). States
      // / provinces with no sites stay light grey. All borders are
      // hairline so the choropleth reads as background, not outline.
      // Status data for the US comes from a curated authority table;
      // Canadian provinces derive both panels from CA_MARKETS.
      const NG_STATUS_US = {
      ...
      // Site-count → fill shading. Sqrt scaling keeps a state with one
      // site visibly tinted while the densest state hits the full
      // status color (with a slight darken so the heaviest markets read
      ...
      const shadeForCount = (statusHex, count) => {
        ...
        // sat: 0.45 (one site) → 1.0 (max sites); dark: 0 → 0.20.
      ...
      // beneath it so the EP-only "Direct Access" chip doesn't show
      // alongside the gas map.
      const SWATCH = 22;
