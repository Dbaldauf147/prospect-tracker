...
import { buildTimelineSvg, STAGE_ICONS } from '../../utils/timelineGraphic';
...
function TimelineVisual({ template }) {
...
function TimelineCard({ template, serviceOptions, filter, onChange, onRemove }) {
  ...
  return (
    <div className={styles.card}>
        ...
        <table className={styles.stageTable}>
          ...
          <tbody>
            {visibleStages.length === 0 ? (
              <tr>
                <td colSpan={7} className={styles.serviceEmpty}>
      ...
      <TimelineVisual template={template} />
    </div>
  );
}
