...
check('a bounce is a delivery failure',
  deliveryStatus(row, watched({ bounced: true }), { sentAt: SENT }), DELIVERY.FAILED);
check('an image load or a click confirms arrival — both happen after delivery',
  deliveryStatus(row, watched(), { hasActivity: true, sentAt: SENT }), DELIVERY.CONFIRMED);
check('sent, watched, nothing bounced, nothing loaded — delivered',
  deliveryStatus(row, watched(), { sentAt: SENT }), DELIVERY.DELIVERED);
...
check('a bounce beats recorded activity',
  deliveryStatus(row, watched({ bounced: true }), { hasActivity: true, sentAt: SENT }),
  DELIVERY.FAILED);

// ---- the honest non-answers ---------------------------------------------

check('no campaign watching means UNKNOWN, not delivered',
  deliveryStatus(row, null, { sentAt: SENT }), DELIVERY.UNKNOWN);
check('...even with no send date to go on',
  deliveryStatus(row, null, {}), DELIVERY.UNKNOWN);
check('watched but never sent is NOT SENT, not delivered',
  deliveryStatus(row, watched(), { sentAt: null }), DELIVERY.NOT_SENT);
...
check('an unwatched send that loaded is still confirmed',
  deliveryStatus(row, null, { hasActivity: true }), DELIVERY.CONFIRMED);
