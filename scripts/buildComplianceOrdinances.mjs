...
// Master Ordinances Database tab (exported to CSV) into the committed seed the
// app bundles:
//
//   src/data/masterOrdinances.js — one record per Government ID, with the raw
//                                  row plus parsed BBS / Audits / BPS
//                                  summaries (status, deadline, thresholds,
//                                  max penalty, key details/links).
//
// The other half of the reference — city+state -> Government ID — comes from
// the workbook's own "City Lookup" tab via scripts/buildCityLookup.mjs. This
// script used to derive a stand-in from each jurisdiction's Government + State
// names, which only ever knew the jurisdiction cities themselves; it no longer
// writes that file, so rebuilding the ordinances can't overwrite the real one.
// Run buildCityLookup.mjs afterwards when the workbook's Government IDs change.
//
// Regenerate:  node scripts/buildComplianceOrdinances.mjs <path-to.csv>
// The raw CSV is not committed; only the processed seed is, so the app runs
// offline.
